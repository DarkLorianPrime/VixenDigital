...
class TestRegistration(TestCase):
    ...
    def test_get_me_user_not_auth(self):
        response = self.client.get(f"{base_url}users/me/")
        self.assertEqual(response.status_code, 401)

    def test_get_me_user_auth(self):
        response = self.client.post(f"{base_url}token/", data=FIRST_USER)
        headers = {"Authorization": f"Bearer {response.data.get('access')}"}
        response = self.client.get(f"{base_url}users/me/", headers=headers)
        self.assertEqual(response.status_code, 200)
